...
const tagColors: Record<TaskTag, { bg: string; text: string }> = {
  '仕事': { bg: 'bg-blue-100', text: 'text-blue-800' },
  'プライベート': { bg: 'bg-green-100', text: 'text-green-800' },
  '学校': { bg: 'bg-yellow-100', text: 'text-yellow-800' },
  'その他': { bg: 'bg-slate-100', text: 'text-slate-800' },
};
...
const MonthlyGoal: React.FC<{
  ...
  return (
    <div className="mb-4 p-4 bg-white rounded-lg border border-slate-200 shadow-md">
      <div className="flex items-center justify-between">
        ...
        {isEditing ? (
          <button onClick={handleSave} className="p-1.5 text-white bg-fuchsia-600 hover:bg-fuchsia-700 rounded-full transition-colors">
             <CheckIcon className="w-4 h-4" />
          </button>
        ) : (
          <button onClick={() => setIsEditing(true)} className="p-1.5 text-slate-500 hover:text-slate-700 hover:bg-slate-100 rounded-full transition-colors">
      ...
      <div className="mt-2">
        {isEditing ? (
           <input
            ...
            className="w-full text-sm px-2 py-1 bg-white border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-fuchsia-500 focus:border-fuchsia-500 transition placeholder:text-slate-400 text-slate-800"
            placeholder="目標を設定しましょう"
            autoFocus
          />
        ) : (
          <p className="text-sm text-slate-600 pl-1 min-h-[26px]">
            {goal || <span className="text-slate-400 italic">目標を設定しましょう</span>}
          </p>
        )}
      </div>
    </div>
...
const Calendar: React.FC<CalendarProps> = ({ selectedDate, onDateChange, tasks, monthlyGoals, onSetGoal, stampedDates }) => {
  ...
  const tasksByDate = useMemo(() => {
    return tasks.reduce((acc, task) => {
      if (!task.isRecurring || task.dueDate.startsWith(`${currentMonth.getFullYear()}-${String(currentMonth.getMonth() + 1).padStart(2, '0')}`)) {
  ...
  const renderDays = () => {
    ...
    for (let i = 0; i < firstDay; i++) {
      days.push(<div key={`empty-${i}`} className="h-24 border-b border-r border-slate-200"></div>);
    }
    ...
    for (let day = 1; day <= totalDays; day++) {
      ...
      let dayClasses = "h-24 flex flex-col p-1.5 cursor-pointer relative transition-colors duration-200 border-b border-r border-slate-200";
      ...
      if (isSelected) {
        ...
        dayNumberClasses += " bg-yellow-400 text-slate-900";
      } else {
      ...
      days.push(
        <div key={day} className={dayClasses} onClick={() => onDateChange(date)}>
          {isStamped && (
            <StarIcon className="absolute top-1.5 right-1.5 w-5 h-5 text-yellow-500" />
          )}
          ...
          <div className="mt-1 overflow-hidden flex-grow space-y-0.5">
            {dailyTasks.slice(0, 2).map(task => {
              const color = tagColors[task.tag] || tagColors['その他'];
              return (
                 <div key={task.id} className={`text-xs truncate px-1.5 py-0.5 rounded ${color.bg} ${color.text} font-medium`}>
                  {task.title}
                </div>
              );
            })}
            {dailyTasks.length > 2 && (
              <div className="text-xs text-slate-400 px-1 py-0.5 font-medium">
  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-200">
      ...
      <div className="grid grid-cols-7 text-center text-sm text-slate-400 font-semibold mb-2">
        {weekdays.map((day, i) => <div key={day} className={`${i === 0 ? 'text-red-500' : i === 6 ? 'text-blue-500' : ''}`}>{day}</div>)}
      </div>
      <div className="grid grid-cols-7 bg-slate-50 border-t border-l border-slate-200 rounded-lg overflow-hidden">
        {renderDays()}
      </div>
    </div>
  );
